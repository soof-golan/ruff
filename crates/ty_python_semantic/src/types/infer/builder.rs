...
impl<'db, 'ast> TypeInferenceBuilder<'db, 'ast> {
    ...
    fn infer_nested_bindings_definition(
        ...
        //         ...
        //
        // By the time we get here in type inference, though, we have everything we need to know to
        // answer this question. We can ask whether `x` was (ever) declared `global` in its scope.
        // If so, or if this is the module scope itself, we see any nested `global` bindings that
        // were recorded. If not, we see any nested `nonlocal` ones. (And if `x` is a free variable
        // in this scope, then these synthetic binding will not shadow `UNBOUND`, so we'll end up
        // walking to defining scope and seeing all the nested bindings from there.)
